...
<html lang="en">
  <head>
  	<title>Task Assign by You</title>
    <meta charset="utf-8">
                        ...
                        <div class="album py-5 bg-body-tertiary">
                            <div class="container">
                                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                                    <?php 
                                    $uid = $_SESSION['uid'];
                                    $selectQuery = "SELECT * FROM task WHERE admin_uid = '$uid' and final_project_status = 'Pending'";
                                    $query = mysqli_query($con, $selectQuery);

                                    if (mysqli_num_rows($query) === 0) {
                                        echo '<p>No tasks assigned By you.</p>';
                                    } else {
                ...
                <div class="completed">
                    <div class="album py-5 bg-body-tertiary">
                                <div class="container">
                                    ...
                                    <?php 
                                    $uid = $_SESSION['uid'];
                                    $selectQuery = "SELECT * FROM task WHERE admin_uid = '$uid' and final_project_status = 'Completed'";
                                    $query = mysqli_query($con, $selectQuery);

                                    if (mysqli_num_rows($query) === 0) {
                                        echo '<p>No tasks Completed assigned By you.</p>';
                                    } else {
                                        while ($result = mysqli_fetch_array($query)) {
                                    ?>
                                    <div class="col">
